{
  "cells": [
    {
      "cell_type": "markdown",
      "metadata": {},
      "source": [
        "# Test 06 — Animation / Showrunner\n",
        "\n",
        "Checks your animation runner wiring.\n",
        "Goal: confirm imports + run a small scripted sequence."
      ]
    },
    {
      "cell_type": "code",
      "metadata": {},
      "outputs": [],
      "source": [
        "from pathlib import Path\n",
        "import sys\n",
        "\n",
        "def add_repo_to_path():\n",
        "    here = Path.cwd().resolve()\n",
        "    for p in [here] + list(here.parents):\n",
        "        if (p / 'lessons').is_dir() and (p / 'common').is_dir():\n",
        "            repo_root = p\n",
        "            common_lib = p / 'common' / 'lib'\n",
        "            if str(repo_root) not in sys.path:\n",
        "                sys.path.insert(0, str(repo_root))\n",
        "            if str(common_lib) not in sys.path:\n",
        "                sys.path.insert(0, str(common_lib))\n",
        "            print('Repo root:', repo_root)\n",
        "            print('Added common/lib:', common_lib)\n",
        "            return\n",
        "    raise FileNotFoundError('Could not find repo root (needs lessons/ and common/)')\n",
        "\n",
        "add_repo_to_path()"
      ]
    },
    {
      "cell_type": "code",
      "metadata": {},
      "outputs": [],
      "source": [
        "import time\n",
        "from robot_moves import RobotMoves\n",
        "\n",
        "rm = RobotMoves()\n",
        "print('RobotMoves ready')\n",
        "\n",
        "print('Simple demo sequence: forward, stop, wiggle turns')\n",
        "rm.drive_for(0.12, 0.0, 0.8)\n",
        "time.sleep(0.3)\n",
        "rm.drive_for(0.0, 1.0, 0.25)\n",
        "rm.drive_for(0.0, -1.0, 0.25)\n",
        "rm.drive_for(0.0, 1.0, 0.25)\n",
        "rm.drive_for(0.0, -1.0, 0.25)\n",
        "rm.stop()\n",
        "print('Done')"
      ]
    }
  ],
  "metadata": {
    "kernelspec": { "display_name": "Python 3", "language": "python", "name": "python3" },
    "language_info": { "name": "python", "version": "3.10" }
  },
  "nbformat": 4,
  "nbformat_minor": 5
}
